...
function create_monitor_ui(bwi,char_name,child_block,enable_map) {
  ...
  if(enable_map) {
    schema.push({name: "minimap", type: "image", label: "Map", options:
      {width: mmap_w, height: mmap_h}, getter:()=>last_beat.mmap});
  }
...
function generate_minimap(game_context) {
  ...
  function relative_coords(x,y) {
    return [(x-c_x)*mmap_scale + mmap_w/2,
  ...
  for(let i = 0; i < geom.x_lines.length;i++) {
    ...
    if(l_x >= mmap_w) break;
    safe_fill_rect(l_x,
      floor((r_y1-c_y)*mmap_scale+mmap_h/2),l_x+1,
      floor((r_y2-c_y)*mmap_scale+mmap_h/2)+1,mmap_cols.wall);
  }
  ...
  for(let i = 0; i < geom.y_lines.length;i++) {
    ...
    safe_fill_rect(
      floor((r_x1-c_x)*mmap_scale+mmap_w/2),l_y,
      floor((r_x2-c_x)*mmap_scale+mmap_w/2)+1,l_y+1,mmap_cols.wall);
  }
  ...
  for(let ent_id in game_context.entities) {
    ...
    let color;
    if(ent.mtype) {
      color = ent.target == g_char.name && mmap_cols.monster_engaged
        || mmap_cols.monster;
    } else {
